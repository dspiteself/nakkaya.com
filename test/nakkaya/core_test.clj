(ns nakkaya.core-test
  ...
  (:use :reload-all nakkaya.core :reload-all)
  (:use :reload-all nakkaya.util :reload-all))
...
(defn request-resource [resource]
...
(deftest test-route-site
  (is (= 200
	 (:status (request-resource "/dummy.markdown"))))
  (is (= {"Content-Type" "text/html"}
	 (:headers (request-resource "/dummy.markdown"))))
  (is (= "<title>dummy content</title>"
	 (re-find #"<title>dummy content</title>"
		  (:body (request-resource "/dummy.markdown")))))
...
(deftest test-route-static
  (is (= 200
	 (:status (request-resource "/dummy/dummy.static"))))
  (is (= (file "public/dummy/dummy.static")
	 (:body (request-resource "/dummy/dummy.static"))))
  (is (= 404
	 (:status (request-resource "/dummy/")))))
...
(deftest test-routes
  ...
  (is (= "b1232b0f-58ce-4339-9272-33fb19da9a12"
	 (re-find #"b1232b0f-58ce-4339-9272-33fb19da9a12"
		  (:body (request-resource "/latest-posts/0/")))))
  ...
  (is (= "January 2050"
	 (re-find #"January 2050"
		  (:body (request-resource "/archives/")))))
  ;;/:year/:month/
  (is (= "b1232b0f-58ce-4339-9272-33fb19da9a12"
	 (re-find #"b1232b0f-58ce-4339-9272-33fb19da9a12"
		  (:body (request-resource "/2050/01/")))))
  (is (= "73c03277-9a03-4fd3-a695-7ff31cd94d92"
	 (re-find #"73c03277-9a03-4fd3-a695-7ff31cd94d92"
		  (:body (request-resource "/2050/01/")))))
  ...
  (is (= "b1232b0f-58ce-4339-9272-33fb19da9a12"
	 (re-find #"b1232b0f-58ce-4339-9272-33fb19da9a12"
		  (:body (request-resource "/2050/01/01/dummy-future-post/")))))
  (is (= "73c03277-9a03-4fd3-a695-7ff31cd94d92"
	 (re-find #"73c03277-9a03-4fd3-a695-7ff31cd94d92"
		  (:body (request-resource "/2050/01/01/dummy-future-post/")))))
  ;;/rss-feed
  (is (= "b1232b0f-58ce-4339-9272-33fb19da9a12"
	 (re-find #"b1232b0f-58ce-4339-9272-33fb19da9a12"
		  (:body (request-resource "/rss-feed")))))
  (is (= "73c03277-9a03-4fd3-a695-7ff31cd94d92"
	 (re-find #"73c03277-9a03-4fd3-a695-7ff31cd94d92"
		  (:body (request-resource "/rss-feed")))))
  ;;/
  ...
  ;;redirect
  (is (= 301
	 (:status (request-resource "/2050/01/01/dummy-future-post"))))
  (is (= {"Location" "/2050/01/01/dummy-future-post/"}
	 (:headers (request-resource "/2050/01/01/dummy-future-post"))))
  (is (= 404
	 (:status (request-resource "/20500101dummy")))))

(deftest test-robot-directives
  (let [regex #"content=\"noindex,follow\".*name=\"robots\""
	match "content=\"noindex,follow\" name=\"robots\""]
    (is (= nil
	   (re-find regex (:body (request-resource "/")))))
    (is (= match
	   (re-find regex
		    (:body (request-resource "/latest-posts/1/")))))
    (is (= match
	   (re-find regex
		    (:body (request-resource "/latest-posts/2/")))))
    (is (= nil
	   (re-find regex
		    (:body (request-resource "/2050/01/01/dummy-future-post/")))))
    (is (= match
