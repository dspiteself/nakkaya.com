...
#+begin_src clojure
  (defn point-to-coordinate [width height [real imag]]
    [(int (+ (* 0.3 width (+ real 0.5)) (/ width 2)))
     (int (+ (* 0.3 height imag) (/ height 2)))])
  
  (defn buffer-set [{width :width height :height buffer :buffer} point]
    (let  [[x y] (point-to-coordinate width height point)]
      (if (and (> x 0)
               (> y 0)
               (< x width)
               (< y height))
...
#+begin_src clojure
  (defn paint-canvas [buffer width height graphics]
  ...
  (defn draw [{buffer :buffer height :height width :width}]
    (let [image  (BufferedImage. width height BufferedImage/TYPE_INT_RGB)
          canvas (proxy [JLabel] []
                   (paint [g] (.drawImage g image 0 0 this)))]
  
      (paint-canvas buffer width height (.createGraphics image))
